...
class LocalServer(port: Int, private val telegramFlow: TelegramFlow) : NanoHTTPD(port) {
        ...
        return runBlocking(Dispatchers.IO) {
            try {
                val filePart = telegramFlow.readFilePart(
                    fileId,
                    superStart.toInt(),
                    min(MAX_CHUNK_SIZE, superSize.getValue(fileId) - superStart.toInt())
                )
                ...
                response.addHeader(
                    "Content-Range",
                    "bytes ${superStart}-${superStart + filePart.data.size - 1}/${superSize}"
                )
    ...
    companion object {
        private const val MAX_CHUNK_SIZE = 6 * 1024 * 1024
    }
}